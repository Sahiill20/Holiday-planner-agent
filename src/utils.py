...
def get_llm(model_type: str = "default"):
    """
    Initializes either the Groq LLM (default) or Hugging Face InferenceClient LLMs based on selection.
    """
    load_dotenv(override=True)
    ...
    model_name = os.environ.get("HF_MODEL_NAME", "llama-3.3-70b-versatile")
    if "groq" in model_name.lower() or "llama-3.3" in model_name.lower():
        from langchain_groq import ChatGroq
        gkey = os.environ.get("GROQ_API_KEY") or os.environ.get("Groq_API_key")
        if not gkey:
            raise ValueError("GROQ_API_KEY was not found in environment variables. Please check your .env file.")
        print(f"[get_llm] Initializing ChatGroq: llama-3.3-70b-versatile | Model Type: {model_type}")
        return ChatGroq(
            model="llama-3.3-70b-versatile",
            groq_api_key=gkey.strip(),
